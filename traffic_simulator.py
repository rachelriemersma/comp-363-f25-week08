...
for t in range(TOTAL_TICKS):
    # for every road in city: advance the cars that are driving on it to stop light
    for src, dst in links:
        pop_to_queue_if_arrived(src, dst)
    # for every intersection in the city: let cars pass through if there is a green light 
    for node in nodes:
        serve_intersection(t, node)
    # for every road in the city...
    for src, dst in links:
        # check if boundry brings cars from outside the city
        if is_boundary_incoming_link(src, dst):
            # generate random number and if its less than the arrival rate create a new car
            if random.random() < ARRIVAL_RATE:
                car_id += 1
                # car object w/ unique id and what tick it entered city on
                new_car = Car(id=car_id, t_enter=t)
                # add car to road with object itsself and amt of ticks to travel the road spawned on
                in_transit[(src, dst)].append((new_car, add_travel_time()))
    # count how many cars are waiting at all the stop lights in the city 
    total_in_queues = 0
    # for every road in the city: count how many cars are stopped 
    for src, dst in links:
        total_in_queues += len(stopped[(src, dst)])
    # add to total (for this tick only)    
    sum_queue += total_in_queues
